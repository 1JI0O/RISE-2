...
import eval_maskaware_json_single as core


default_args = deepcopy(core.default_args)
default_args["save_root"] = "deploy_capture_single"
...
def _build_capture_dirs(save_root):
    ...
    mask_dir = os.path.join(session_root, "mask")
    ...
    for path in [rgb_dir, depth_dir, mask_dir, action_dir, proprio_dir, joint_dir]:
...
def evaluate(args_override):
    ...
    if str(config.robot_type) != "single":
        raise ValueError(
            f"eval_maskaware_json_single_260314_save.py only supports robot_type=single, got {config.robot_type}"
        )
    ...
    with core._auto_input_when_debug(debug_enabled):
        if args.type == "local":
            ...
            raise NotImplementedError(
                "remote inference is not wired in this standalone single-arm capture eval script"
            )
        ...
        agent = None
        try:
            agent = core._build_agent(args, config)
            core._log_mask_aware_summary(config.mask_aware)

            core._arm_renderer = (
                core._init_mask_renderer(config.mask_aware, agent)
                if args.type == "local"
                else None
            )
            ...
            with torch.inference_mode():
                for t in range(rollout_steps):
                    ...
                    latest_mask01 = (
                        None if mask01 is None else np.asarray(mask01).copy()
                    )
