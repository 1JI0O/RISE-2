...
from dataset.realworld_tcp_anchor import RealWorldDataset, collate_fn
...
def train(args_override):
    ...
    if RANK == 0:
        ...
        if config.data.sample_traj:
            print(
                "Enabling trajectory sampling, this may take a while during dataset init."
            )
    ...
    if args.resume_ckpt is not None:
        ...
        if RANK == 0:
            print(
                "Checkpoint {} from step {} loaded.".format(
                    args.resume_ckpt, args.resume_step
                )
            )
    ...
    optimizer = torch.optim.AdamW(
        policy.parameters(), lr=config.train.lr, betas=[0.95, 0.999], weight_decay=1e-6
    )
        ...
        for data in pbar:
            ...
            if (cur_step + 1) % config.train.save_steps == 0:
                torch.save(
                    ...
                    os.path.join(
                        args.ckpt_dir,
                        "policy_step_{}_seed_{}.ckpt".format(
                            cur_step + 1, config.train.seed
                        ),
                    ),
                )
    ...
    if RANK == 0:
        torch.save(
            policy.module.state_dict(), os.path.join(args.ckpt_dir, "policy_last.ckpt")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", action="store", type=str, help="data path", required=True
    )
    parser.add_argument(
        "--ckpt_dir",
        action="store",
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="data and model config during training and deployment",
        required=True,
    )
    parser.add_argument(
        "--resume_ckpt",
