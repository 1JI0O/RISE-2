...
from dataset.realworld_original import RealWorldDataset, collate_fn
...
def train(args_override):
    ...
    if RANK == 0:
        ...
        sample_traj_cfg = getattr(config.data, "sample_traj", None)
        sample_traj_enable = bool(getattr(sample_traj_cfg, "enable", False))
        if sample_traj_enable:
    ...
    # dataset & dataloader
    if RANK == 0:
        print("Loading dataset from dataset.realworld_original ...")
    ...
    for epoch in range(resume_epoch + 1, num_epochs):
        ...
        for data in pbar:
            ...
            # vanilla training path: no image_mask_weight
            loss = policy(cloud_data, image, image_coords, image_mask_weight = None, actions = action_data)
