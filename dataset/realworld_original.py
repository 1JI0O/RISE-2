...
class RealWorldDataset(Dataset):
    ...
    def _parse_config(self, config):
        ...
        if calib_timestamp in [None, "", "null", "None"]:
    ...
    def register_projector(self, demo_path, calib_path):
        # 优先使用 config.data.calib_timestamp
        if self.config_calib_timestamp is not None:
            calib_timestamp = self.config_calib_timestamp
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.vis:
            ...
            save_prefix = f"{scene_name}_{cam_id}_{obs_frame_id}"
            vis_data(
                points,
